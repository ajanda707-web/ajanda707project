...
-- Update  : - Folder Structure (PrawiraHubSambungKata/Database/...)
--           - Strict Length Filter (< 3 Huruf = Permanent Blacklist)
--           - Auto Clean LocalDB (Hapus duplikat vs Github DB)
--           - Menggunakan logic VIM KeyCode untuk HP & PC (Suara ketik on)
-- =================================================================
...
local function RegisterWord(w)
    ...
    -- [STRICT FILTER] Jika huruf berjumlah 2 atau kurang -> BLACKLIST OTOMATIS
    if #wl < 3 then
        BlacklistDB[wl] = true
...
local BtnReset            = makeRiwBtn(LY+298, "⟳ Kosongkan Data Use Ronde", THEME.BtnReset)
...
local BtnDel  = makeSmBtn(61, "Clr DB",  THEME.BtnDelete, 5)
...
local CloseOverlay, CloseScale = createOverlay("Are you sure you want to close?", function()
    ...
    KamusDict,WordCache,UsedWords,TempIgnored,GithubDict={},{},{},{},{}
...
local function ForceClearDataUse()
    if next(UsedWords) ~= nil or next(TempIgnored) ~= nil then
        ...
        UpdateDBStatUI()
        if TriggerListRefresh then TriggerListRefresh() end
        if LblStatus then LblStatus.Text="Data Use Di-reset!"; LblStatus.TextColor3=THEME.Cyan end
    end
end
...
BtnImp.MouseButton1Click:Connect(function()
    ...
    if ok and type(data)=="table" then
        ...
        for k,v in pairs(data) do
            if not LocalDB[k] and #tostring(k) > 2 then
