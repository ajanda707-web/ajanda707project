-- =================================================================
-- Script  : SAMBUNG KATA AUTO PLAY V2 (HP & PC FULL SUPPORT)
-- Author  : PrawiraXLIV
-- Support : PC (VIM KeyCode) & Mobile/HP (Absolute Click UI Button)
-- Update  : - HP mengetik via TextButton agar player lain dengar suaranya
--           - Output Logger disesuaikan persis dengan request
-- =================================================================
...
local function RegisterWord(w)
    if type(w) ~= "string" or #w < 3 then return false end
    local wl = w:lower()
...
-- ============================================================
-- getPrefix (MEMBACA TURNBILLBOARD TERLEBIH DAHULU, BACKUP WORDSERVER)
-- ============================================================
local function getPrefix()
    local prefix = nil
    pcall(function()
        -- Prioritas Utama: Baca langsung dari Billboard di atas kepala player saat giliran
        ...
        -- Prioritas Backup: Baca dari MatchUI Lokal (WordServer)
        if not prefix then
...
-- ============================================================
-- 📱 MOBILE / HP KEYBOARD CACHE (ABSOLUTE POSITION CLICK)
-- Fungsi ini akan meng-cache seluruh TextButton asli yang ada
-- di Row1 - Row4 UI Game sehingga klik akan men-trigger event asli
-- (MatchLocalScript2 akan mengeluarkan suara ngetik ke server)
-- ============================================================
local _kbButtonCache = nil
local function InvalidateKeyboardCache() _kbButtonCache = nil end

local function GetKeyboardButtons()
    if _kbButtonCache then
        local tb = _kbButtonCache["enter"] or _kbButtonCache["a"]
        if tb and tb.Parent and tb.Parent.Parent then return _kbButtonCache end
        _kbButtonCache = nil
    end
    local result = {}
    pcall(function()
        local pg = LocalPlayer:FindFirstChild("PlayerGui"); if not pg then return end
        local mUI = pg:FindFirstChild("MatchUI"); if not mUI then return end
        local bUI = mUI:FindFirstChild("BottomUI"); if not bUI then return end
        local kb  = bUI:FindFirstChild("Keyboard"); if not kb then return end
        
        -- Cari di dalam Row1, Row2, Row3, Row4
            ...
            if row:IsA("Frame") and row.Name:match("^Row%d") then
                for _, btn in ipairs(row:GetChildren()) do
                    if btn:IsA("TextButton") then
                        ...
                        -- Lookup berdasakan Name terlebih dahulu (lebih presisi)
                        if nm == "backspace" then result["backspace"] = btn
                        elseif nm == "enter" then result["enter"] = btn
                        elseif #nm == 1 and nm:match("^%a$") then result[nm] = btn
                        else
                            -- Backup pakai Text
                            local tx = tostring(btn.Text):lower():gsub("%s+","")
                            if #tx == 1 and tx:match("^%a$") then result[tx] = btn
                            elseif tx == "backspace" or tx == "⌫" or tx == "del" then result["backspace"] = result["backspace"] or btn
                            elseif tx == "enter" or tx == "return" or tx == "ent" then result["enter"] = result["enter"] or btn
                            end
                        end
                    end
                end
    ...
    if next(result) then _kbButtonCache = result end
    return result
end

-- Simulasi ketukan jari di layar (Absolute Position Center Button)
local function ClickButton(btn)
    if not btn or not btn.Parent then return false end
    local ok = false
    pcall(function()
        local pos = btn.AbsolutePosition; local sz = btn.AbsoluteSize
        local cx = math.floor(pos.X + sz.X * 0.5)
        local cy = math.floor(pos.Y + sz.Y * 0.5)
        VIM:SendMouseButtonEvent(cx, cy, 0, true, game, 1); task.wait(0.035)
        VIM:SendMouseButtonEvent(cx, cy, 0, false, game, 1); ok = true
    end)
    return ok
end

local function MobilePressKey(c, back, enter)
    local btns = GetKeyboardButtons(); if not btns or not next(btns) then return false end
    local key = back and "backspace" or enter and "enter" or (c and tostring(c):lower() or nil)
    local btn = key and btns[key]
    
    if not btn then 
        InvalidateKeyboardCache(); btns = GetKeyboardButtons(); btn = key and btns and btns[key] 
    end
    
    if btn then return ClickButton(btn) end
    if LblStatus then LblStatus.Text="⚠️ Tombol '"..tostring(key).."' tidak ditemukan di UI!"; LblStatus.TextColor3=THEME.Red end
    return false
end
...
-- 🖥️ PC KEYBOARD VIM
-- ============================================================
local function PCPressKey(c, back, enter)
    local kc = nil
    if back then kc = Enum.KeyCode.Backspace
    elseif enter then kc = Enum.KeyCode.Return
    elseif c and c~="" then pcall(function() kc = Enum.KeyCode[tostring(c):upper()] end) end
    
    if kc then VIM:SendKeyEvent(true, kc, false, game); task.wait(0.015); VIM:SendKeyEvent(false, kc, false, game) end
end

-- ============================================================
-- TypeSingleWord (AMU CRASH METHOD)
-- ============================================================
local function TypeSingleWord(kata, passedPrefix)
    local safePrefix = tostring(passedPrefix or "")
    if safePrefix == "" then
        LblStatus.Text = "❌ Gagal: Prefix kosong!"
        return false
    end

    if currentPlayerTurn and currentPlayerTurn ~= LocalPlayer then
        LblStatus.Text = "❌ TERTUNDA: Giliran Orang Lain!"; LblStatus.TextColor3 = THEME.Red
        return false
    end
    
    local wKasar = tostring(kata)
    LblTyping.Text = "TARGET: "..wKasar:upper()
    LblStatus.Text = "Menghapus sisa teks..."; LblStatus.TextColor3 = THEME.Yellow
    
    -- Pastikan UI Button HP tersimpan di cache
    if isMobile then GetKeyboardButtons() end

    local function pressKey(c, back, enter)
        local ok = pcall(function()
            if isMobile then MobilePressKey(c,back,enter) else PCPressKey(c,back,enter) end
        end)
        return ok
    end

    -- Menghapus sisa ketikan di box
    local hapusSpd = tonumber(DeleteSpeed) or 0.05
    for _ = 1, 8 do
        if currentPlayerTurn and currentPlayerTurn~=LocalPlayer then return false end
        pressKey("", true, false); task.wait(hapusSpd)
    end
    task.wait(0.05)

    LblStatus.Text = "Mencoba: "..wKasar:upper(); LblStatus.TextColor3 = THEME.Cyan
    
    -- Menghapus Prefix dari target kata
    local sisa = ""
    pcall(function()
        local escapePrefix = safePrefix:gsub("([^%w])", "%%%1")
        sisa = wKasar:gsub("^" .. escapePrefix, "", 1)
    end)
    
    wordStatus = "waiting"

    -- Proses Mengetik (Mensimulasikan PC / Click TextButton HP)
    local ketikSpd = tonumber(TypingSpeed) or 0.05
    local lenSisa = string.len(sisa)
    for i = 1, lenSisa do
        if currentPlayerTurn and currentPlayerTurn~=LocalPlayer then return false end
        local charToType = ""
        pcall(function() charToType = string.sub(sisa, i, i) end)
        pressKey(charToType, false, false)
        task.wait(0.015); task.wait(ketikSpd)
    end
    task.wait(0.05)
    
    -- Tekan Enter untuk mensubmit
    pressKey("", false, true); task.wait(0.05)

    local timeout = tick() + 1.5
    while tick() < timeout and scriptActive do
        task.wait(0.05)
        local checkPref = getPrefix()
        if wordStatus=="correct" or wordStatus=="used" or (checkPref and checkPref ~= safePrefix) then break end
    end

    local finalCheckPref = getPrefix()
    if wordStatus=="correct" or (finalCheckPref and finalCheckPref ~= safePrefix) then
        UsedWords[wKasar] = true
        lastRiwayatWord = wKasar; UpdateRiwayatUI()
        LblStatus.Text = "✓ BENAR: "..wKasar:upper(); LblStatus.TextColor3 = THEME.Neon
        
        if RegisterWord(wKasar) then LocalDB[wKasar]=true; SaveDatabases()
        else totalDuplicates=totalDuplicates+1 end
        
        UpdateDBStatUI()
        -- Reset cache jika HP untuk load posisi UI yang baru jika bergeser
    ...
    elseif wordStatus=="used" then
        UsedWords[wKasar] = true
        LblStatus.Text = "⚠️ SUDAH TERPAKAI: "..wKasar:upper(); LblStatus.TextColor3 = Color3.fromRGB(255,150,50)
        for _ = 1, lenSisa do pressKey("", true, false); task.wait(0.015); task.wait(hapusSpd) end
        UpdateDBStatUI()
        if TriggerListRefresh then TriggerListRefresh() end
        task.wait(0.1); return false

    else
        BlacklistDB[wKasar] = true; if LocalDB[wKasar] then LocalDB[wKasar]=nil end
        SaveDatabases()
        LblStatus.Text = "❌ TIDAK VALID (BLACKLIST)!"; LblStatus.TextColor3 = THEME.Red
        for _ = 1, lenSisa do pressKey("", true, false); task.wait(0.015); task.wait(hapusSpd) end
        UpdateDBStatUI()
        if TriggerListRefresh then TriggerListRefresh() end
        task.wait(0.1); return false
...
local function ClearMatchUseData()
    if next(UsedWords) ~= nil then
        UsedWords = {}; InvalidateKeyboardCache(); UpdateDBStatUI()
        if TriggerListRefresh then TriggerListRefresh() end
        if LblStatus then LblStatus.Text="Ronde Selesai. Data Use Di-reset!"; LblStatus.TextColor3=THEME.Cyan end
    end
end

table.insert(scriptConnections, LocalPlayer:GetAttributeChangedSignal("CurrentTable"):Connect(function()
    if not LocalPlayer:GetAttribute("CurrentTable") then ClearMatchUseData(); InvalidateKeyboardCache() end
end))
if remotes and remotes:FindFirstChild("ResultUI") then
    table.insert(scriptConnections, remotes.ResultUI.OnClientEvent:Connect(function()
        ClearMatchUseData(); InvalidateKeyboardCache()
    end))
end
...
mainThread = task.spawn(function()
    ...
    while scriptActive do
        ...
        if not isJoinedFast() then
            ...
            if lastPrefix~=nil then lastPrefix=nil; InvalidateKeyboardCache(); setScrollPlaceholder("Silakan Join Meja.", THEME.Neon) end
            continue
        end
        ...
        if prefix then
            if prefix~=lastPrefix then
                ...
                if isMobile then task.spawn(function() GetKeyboardButtons() end) end
...
BtnImp.MouseButton1Click:Connect(function()
    ...
    if ok and type(data)=="table" then
        ...
        for k,v in pairs(data) do
            if not LocalDB[k] and not KamusDict[k] and not BlacklistDB[k] and #tostring(k)>2 then
...
BtnImpB.MouseButton1Click:Connect(function()
    ...
    if ok and type(data)=="table" then
        ...
        for k,_ in pairs(data) do
            if not BlacklistDB[k] and #tostring(k)>2 then
...
task.spawn(function()
        ...
        if ok and res then
            for line in string.gmatch(res,"[^\r\n]+") do
                ...
                if w and #w>2 then
                    local wl = w:lower()
                    GithubDict[wl] = true
                    if RegisterWord(w) then totalKata=totalKata+1 end
                end
            end
        end
    end
    ...
    for k,_ in pairs(LocalDB) do
        if GithubDict[k] then LocalDB[k]=nil; cleanCnt=cleanCnt+1; totalDuplicates=totalDuplicates+1 end
    end
    if cleanCnt>0 then SaveDatabases() end

    dbLoaded = true
    Title.Text = "PrawiraHub - Sambung Kata"
    LblStatus.Text = "Siap! "..math.floor(totalKata/1000).."k+ kata"..(cleanCnt>0 and " | Dibersihkan "..cleanCnt.." duplikat LocalDB" or "")
