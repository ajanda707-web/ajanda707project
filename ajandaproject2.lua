...
-- Author  : PrawiraXLIV
-- Support : PC & Mobile (HP) - ABSOLUTE CRASH FREE (V2)
-- =================================================================
...
table.insert(scriptConnections, game:GetService("LogService").MessageOut:Connect(function(msg, msgType)
    if isOutputOn then AddLog("[" .. tostring(msgType.Name) .. "] " .. msg) end
end))
...
-- ============================================================
-- MOBILE KEYBOARD CACHE (LOGIC ASLI)
-- ============================================================
...
local function ClickButton(btn)
    ...
    pcall(function()
        ...
        local cx=math.floor(pos.X+sz.X*0.5); local cy=math.floor(pos.Y+sz.Y*0.5)
        VIM:SendMouseButtonEvent(cx,cy,0,true,game,1); task.wait(0.04)
...
local function MobilePressKey(c, back, enter)
    ...
    if not btn then InvalidateKeyboardCache(); btns=GetKeyboardButtons(); btn=key and btns and btns[key] end
    if btn then return ClickButton(btn) end
    if LblStatus then LblStatus.Text="⚠️ Tombol '"..tostring(key).."' tidak ditemukan!"; LblStatus.TextColor3=THEME.Red end
    return false
end
...
local function TypeSingleWord(kata, passedPrefix)
    ...
    if safePrefix == "" then
        LblStatus.Text = "❌ Gagal: Prefix kosong/nil!"
        return false
    end
    ...
    for _ = 1, 8 do
        -- Kita hanya cek turn, jangan paksa panggil getPrefix lagi karena bisa hilang mendadak
    ...
    -- Menggunakan string.gsub untuk menghapus prefix di awal kata
    -- Ini adalah metode paling aman yang tidak terpengaruh error 'sub'
    local sisa = ""
    pcall(function()
    ...
    for i = 1, lenSisa do
        ...
        task.wait(0.02); task.wait(ketikSpd)
    end
    task.wait(0.05)
    ...
    if wordStatus=="correct" or (finalCheckPref and finalCheckPref ~= safePrefix) then
        ...
        UpdateDBStatUI(); InvalidateKeyboardCache()
        if TriggerListRefresh then TriggerListRefresh() end
