...
-- Update  : - Menggunakan logic VIM KeyCode v46.38 (Zero-Typos di HP/PC)
--           - Game merespon VIM di HP (Suara ketik terdengar player lain)
--           - Format Output Logger disamakan persis dengan request
-- =================================================================
...
local CloseOverlay, CloseScale = createOverlay("Are you sure you want to close?", function()
    ...
    KamusDict,WordCache,UsedWords,TempIgnored,BlacklistDB={},{},{},{},{}
...
table.insert(scriptConnections, LocalPlayer:GetAttributeChangedSignal("CurrentTable"):Connect(function()
    if not LocalPlayer:GetAttribute("CurrentTable") then ForceClearBlacklist() end
end))

local remotes = ReplicatedStorage:WaitForChild("Remotes", 5)
if remotes and remotes:FindFirstChild("ResultUI") then
    table.insert(scriptConnections, remotes.ResultUI.OnClientEvent:Connect(function() ForceClearBlacklist() end))
end
...
local function getPrefix()
    local prefix = nil
    pcall(function()
        -- Prioritas 1: Baca langsung dari Billboard di atas kepala player saat giliran
        ...
        -- Prioritas 2: Baca dari MatchUI Lokal (WordServer)
        if not prefix then
...
-- Metode ini di-ekstrak murni dari v46.38 karena terbukti
-- kompatibel untuk PC dan HP (memainkan sound via ContextAction)
-- ============================================================
local function TypeSingleWord(kata, passedPrefix)
    ...
    -- Hapus persiapan instan (15x backspace secepat kilat)
    for b = 1, 15 do
    ...
    -- Proses Mengetik (VirtualInputManager sangat aman untuk Mobile di game ini)
